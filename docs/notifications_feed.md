...
2. **In-app FMoIP actions** — Controlled by JSON fields on the same item (no separate `fmoip://` URI is required today):
   ...
   | `highlight` | `true` / `false`. Works for **both** **`read`** and **`write`**. **`read` + `highlight: false`:** does **not** open Settings for mismatch UI, does **not** blink or outline anything, and clears any leftover notification emphasis from a previous item. When `true`, the app uses **`kv`** and the **current** app settings to decide what to show. **`write` + `highlight: true`:** applies prefs from **`kv`**, then opens **Settings** with a **steady** outline on every highlighted row (and home-only keys) until the user leaves Settings or taps the control. **`read` + `highlight: true`:** does **not** apply prefs. If a key is present with **no concrete value** (empty string, JSON `null`, or the literal `null`), the app **does not** compare to the current setting: that row gets a **steady** outline only (no blink). If a **non-empty** value is given and it **does not match** after normalization, that row **blinks**. Matching non-empty values get **no** outline. Scroll target: **`_focus`** among attention keys (mismatches + value-omitted keys), else first such Settings row, else **Station catalog** if none apply. |
...
**Redundant prefs (`read` and `write`):** For **`action: read`** or **`action: write`**, the app compares the **`kv`** object (excluding **`_*`** meta keys) to the user’s **current** snapshot of those keys. Keys with **no concrete value** (same rules as **read** “ignore value”: empty, JSON null, literal `null`) are **skipped** for this check. If **every remaining** key **already matches**, the notification is **hidden** (no popup, bell row, or unread count)—including **`retention: history`** items once prefs match after **Interact** or other changes. If **only** value-omitted keys are present, the item is **not** treated as redundant. Until persisted settings have loaded (`settingsPrefsLoaded`), the snapshot may be incomplete and an item can appear briefly.
...
- **Point users at Settings without changing prefs:** `"action": "read"`, `"highlight": true`, and **`kv`** with the **target** values you want them to match; controls **blink** only where the app still differs. Omit **`kv`** or use `"kv": {}` to open **Settings** scrolled to **catalog** with no emphasis.
